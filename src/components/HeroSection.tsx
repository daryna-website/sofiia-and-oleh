    ...
    <section className="relative w-full h-screen min-h-[600px] max-h-[1070px] flex flex-col items-center justify-center overflow-hidden">
      ...
      <div className="absolute top-8 right-8 md:top-14 md:right-14 z-10">
        <div className="flex items-center bg-white/15 backdrop-blur-md rounded-full p-1 border border-white/20">
          <button
            onClick={() => setLang('ua')}
            className={`font-body font-medium text-sm md:text-base px-4 py-1.5 rounded-full transition-colors ${lang === 'ua' ? 'text-white bg-white/25' : 'text-white/50 hover:text-white/80'}`}>
            
            UA
          </button>
          <button
            onClick={() => setLang('it')}
            className={`font-body font-medium text-sm md:text-base px-4 py-1.5 rounded-full transition-colors ${lang === 'it' ? 'text-white bg-white/25' : 'text-white/50 hover:text-white/80'}`}>
      ...
      <div className="relative z-10 flex flex-col items-center mt-auto mb-12 md:mb-20 px-4 text-center">
        <h1
          ...
          style={{
            textShadow: '0px 2px 32px rgba(0,0,0,0.2)'
          }}>
        ...
        <p className="font-body text-lg md:text-[20px] text-white">
          06.06.2026
        </p>
      </div>
    </section>);
